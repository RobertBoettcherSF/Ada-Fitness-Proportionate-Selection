--  Fitness_Proportionate_Selection body — classic roulette-wheel FPS.

pragma Ada_2022;

package body Fitness_Proportionate_Selection
  with SPARK_Mode => Off
is
   ...
   ---------------------------------------------------------------------------
   -- Fitness validation / shift / probability
   ---------------------------------------------------------------------------
   ...
   begin
      ...
      if Index < Pop'First or else Index > Pop'Last then
         raise Invalid_Argument;
      end if;
      ...
      return Unit_Interval (Pop (Index).Fitness / F);
   end Selection_Probability;
   ...
   is
      Idx : constant Positive := Select_One (Pop, State);
   begin
      return Pop (Idx);
   end Select_One;
   ...
   is
   begin
      if not Valid_Config (Cfg, Pop'Length, 1) then
         raise Invalid_Argument;
      end if;
      Seed_RNG (State, Cfg);
      return Select_One (Pop, State);
   end Select_One;
   ...
   is
      Idx    : constant Index_List := Select_Parents (Pop, Count, State);
   ...
   is
   begin
      if not Valid_Config (Cfg, Pop'Length, Natural (Count)) then
         raise Invalid_Argument;
      end if;
      Seed_RNG (State, Cfg);
      return Select_Parents (Pop, Count, State);
   end Select_Parents;
