--  Fitness_Proportionate_Selection — Ada 2023 educational package for
--  Wikipedia "Fitness proportionate selection" (roulette-wheel /
--  spinning-wheel): select parents with probability proportional to
--  non-negative fitness via a cumulative wheel and independent
--  Uniform(0, F) draws. Fitness must be non-negative; costs can be
--  shifted to fitness via Shift_Costs_To_Fitness.
--  Primary source:
--  https://en.wikipedia.org/wiki/Fitness_proportionate_selection
--  Siblings: Ada-Stochastic-Universal-Sampling (lower spread);
--  Ada-Tournament-Selection; Ada-Truncation-Selection
--  (README links; no package deps).

pragma Ada_2022;

package Fitness_Proportionate_Selection
  with SPARK_Mode => Off
is
   ...
   --  non-negative for Select_One / Select_Parents.
   ...
   --         Seed_RNG convenience). Selection count is passed per call.
   ...
   --  True when Pop_Size ≥ 1 and Select_Count ≥ 1.
   ...
   --  Selection probability p_i = f_i / Σ_j f_j for Index within Pop.
   --  Raises Invalid_Argument if Pop empty, Index out of range,
   --  any Fitness < 0, or total fitness F = 0.
   function Selection_Probability
     (Pop : Population; Index : Positive) return Unit_Interval
     with Global => null;
   ...
   ---------------------------------------------------------------------------
   -- Cumulative wheel / pointer map (RWS)
   ---------------------------------------------------------------------------
   ...
   --  One independent roulette draw: U ~ Uniform(0, F), map through
   --  the cumulative wheel. Returns the selected population index.
   --  Raises Invalid_Argument if Pop empty, any Fitness < 0, or F = 0.
   function Select_One
     (Pop   : Population;
      State : in out RNG_State) return Positive
     with Global => null;

   --  Same draw, returning the Individual record.
   function Select_One
     (Pop   : Population;
      State : in out RNG_State) return Individual
     with Global => null;

   --  Convenience: seed from Cfg then Select_One (index).
   function Select_One
     (Pop   : Population;
      Cfg   : Config;
      State : in out RNG_State) return Positive
   ...
   --  Count independent Uniform(0, F) draws mapped by RWS.
   --  Higher sampling variance than Baker SUS (evenly spaced pointers).
   --  Raises Invalid_Argument if Pop empty, Count < 1 path via Positive,
   --  any Fitness < 0, or F = 0.
   ...
   --  Convenience: seed from Cfg then Select_Parents (indices).
   function Select_Parents
     (Pop   : Population;
      Count : Positive;
      Cfg   : Config;
...
end Fitness_Proportionate_Selection;
